...
public class MeatHookRenderer implements BlockEntityRenderer<MeatHookBlockEntity> {
    ...
    @Override
    public void render(MeatHookBlockEntity pBlockEntity,
                       float pPartialTick,
        ...
        if (pBlockEntity.hasLevel()) {
			...
			pBlockEntity.getCurrentTool().ifPresent(animatedRecipeItemUse -> animatedRecipeItemUse.model.forEach(b -> RenderUtil.loadModel(pPoseStack, pBufferSource, pPackedLight, pPackedOverlay, b, timer)));
            pPoseStack.popPose();
            timer++;
        }
    }
